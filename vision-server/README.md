...
새 미니게임 [7. 소리지르기](../docs/minigames/07_소리지르기.md)는 카메라 포즈/얼굴이 아니라
마이크 음량만 입력으로 쓴다. 아직 이 서버는 마이크를 전혀 캡처하지 않는다 — 온라인 모드에서는
플레이어별로 이 프로세스가 이미 그 사람의 PC에서 돌고 있으므로, 카메라 캡처 루프와 별개로
마이크 입력을 열어 RMS/dB를 계산하고 `shared/PROTOCOL.md`의 `voice.level` 필드로 같이 실어
보내는 방향을 제안한다. 구체적인 라이브러리 선택과 정규화 공식은 아직 미정 —

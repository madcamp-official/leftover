...
캐릭터로 5개 미니게임(돌던지기, 점프해서 과일따기, 머리로 코코넛 깨기, 돌 or 바나나,
눈빛싸움)에서 1:1로 겨루는 파티 게임. 기획: [docs/우가우가게임_기획_프롬프트.md](docs/우가우가게임_기획_프롬프트.md)

> 미니게임 2종([소리지르기](docs/minigames/07_소리지르기.md),
> [깃털날기](docs/minigames/08_깃털날기.md))이 추가로 기획됐다 — 문서화만 끝난 단계로
> 아직 씬/스크립트는 없다. 소리지르기는 마이크 음량이라는 새 입력 신호가 필요해서
> `shared/PROTOCOL.md`/`vision-server` 쪽 확장이 먼저 필요하다(각 문서 13장 TODO 참고).
>
...
지금은 로컬(LAN)에서만 동작한다 — 실행파일 하나로 배포하고 인터넷 너머 방코드로 접속하는
목표 아키텍처는 [docs/배포_아키텍처_설계.md](docs/배포_아키텍처_설계.md)(설계 단계, 미구현) 참고.
